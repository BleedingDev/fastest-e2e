...
const run = Tool.make("browser_run", { description: "Execute a bounded browser task in the configured profile. Returns a durable run, requested extraction and evidence. Never put secrets in the goal; use environment references for input.", parameters: Task, success: ReportSchema, failure: BrowserError }).annotate(Tool.Destructive, true).annotate(Tool.OpenWorld, true);
const test = Tool.make("browser_test", { description: "Execute UI steps or an autonomous goal and verify declared expectations. A model completion claim is not evidence. Preserves failed attempts.", parameters: TestTask, success: ReportSchema, failure: BrowserError }).annotate(Tool.Destructive, true).annotate(Tool.OpenWorld, true);
...
const resume = Tool.make("browser_resume", { description: "Continue a run after validating its revision and live state, or explicitly reconstruct/restart when preauthorized and safe. Never blindly repeat uncertain mutations.", parameters: Resume, success: ReportSchema, failure: BrowserError }).annotate(Tool.Destructive, true).annotate(Tool.OpenWorld, true);
